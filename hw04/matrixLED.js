...
for(var j=7; j>=0; j--) {
	matrixData_R += '<tr>';
	for(var i=0; i<8; i++) {
	    matrixData_R += '<td><div class="LED" id="id'+i+'_'+j+'_R'+
		'" onclick="LEDclick_R('+i+','+j+')">'+
		i+','+j+'</div></td>';
	    }
	matrixData_R += '</tr>';
}
$('#matrixLED_R').append(matrixData_R);
...
function LEDclick_R(i, j) {
//	alert(i+","+j+" clicked");
    disp_R[i] ^= 0x1<<j;
    socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i + 1, 
			     disp: '0x'+disp_R[i].toString(16)});
//	socket.emit('i2c', i2cNum);
    // Toggle bit on display
    if(disp_R[i]>>j&0x1 === 1) {
        $('#id'+i+'_'+j+'_R').addClass('on_R');
    } else {
        $('#id'+i+'_'+j+'_R').removeClass('on_R');
    }
}
    ...
    */
    ...
    function matrix(data) {
        ...
        for (i = 0; i < data.length; i += 2) {
            ...
            disp_R[i / 2] = parseInt(data[i+1], 16);
        }
